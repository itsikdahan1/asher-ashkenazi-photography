          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-2 gap-4 relative z-10">
              {packaging.images.map((imageUrl, index) => (
                <img 
                  ...
                  src={imageUrl} 
                  alt={packaging.title} 
                  className={`rounded-2xl shadow-2xl border border-white/10 w-full h-48 object-cover ${index % 2 === 0 ? 'translate-y-4' : '-translate-y-4'}`}
                />
              ))}
            </div>
            {(packaging.badgeTitle || packaging.badgeText) && (
              <div className="absolute -bottom-10 -right-4 bg-charcoal p-6 rounded-2xl border border-white/10 shadow-2xl z-20 hidden lg:block">
                ...
                {packaging.badgeText && <span className="text-cream/60 text-xs">{packaging.badgeText}</span>}
              </div>
            )}
          </div>
        </motion.div>
